import { CalendarDays, CheckCircle2, ChefHat, ClipboardList, LogOut, Plus, Send, Soup, Table2, Trash2, UserCheck, UsersRound, Utensils } from "lucide-react";
...
type CustomerView = "daily" | "menu" | "tracking" | "people";
type MenuEntry = {
  date: string;
  items: string[];
...
export function CompanyMealPortal({ companyCode }: Props) {
  ...
  const monthlyAmounts = calculateMealAmount(monthlyTotalHeadcount, invoiceRequested);
    ...
    return [...totals.values()]
      .map((total) => {
        ...
        return {
          ...
          total: calculateMealAmount(dates.length, invoiceRequested).grossTotal
        };
      })
      .sort((first, second) => first.name.localeCompare(second.name, "tr-TR"));
  }, [monthlyRequests, invoiceRequested]);
        ...
        <section className="monthly-menu-section customer-monthly-tracking" id="aylik-takip">
          <div className="panel-title-row">
            ...
            <div className="customer-billing-actions">
              <button className="invoice-toggle-button" type="button" onClick={() => setInvoiceRequested((current) => !current)}>
                <CheckCircle2 size={17} />
                {invoiceRequested ? `Fatura kesilsin +%${VAT_RATE * 100} KDV` : "Faturasız ödeme"}
              </button>
              <label className="dashboard-date-filter">
                <CalendarDays size={17} />
          ...
          <div className="customer-tracking-summary">
            ...
            <article>
              ...
              <small>{formatCurrency(MEAL_UNIT_PRICE)} x kişi</small>
            </article>
            <article>
              <span>KDV</span>
              <strong>{formatCurrency(monthlyAmounts.vatTotal)}</strong>
              <small>{invoiceRequested ? `Fatura için %${VAT_RATE * 100}` : "Faturasızda eklenmez"}</small>
            </article>
            <article className="highlight">
              <span>Ay sonu tutar</span>
              <strong>{formatCurrency(monthlyAmounts.grossTotal)}</strong>
              <small>{invoiceRequested ? "Faturalı toplam" : "Faturasız ödeme"}</small>
            </article>
          </div>
            ...
            {isMonthlyLoading ? (
              ...
              monthlyRequests.map((monthlyRequest) => (
                <div className="customer-tracking-row" key={monthlyRequest.requestNo}>
                  ...
                  <strong>{formatCurrency(calculateMealAmount(monthlyRequest.headcount, invoiceRequested).grossTotal)}</strong>
                  <em>
        ...
        <section className="monthly-menu-section company-people-section" id="kisiler">
          ...
          <div className="person-billing-panel">
            <div className="panel-title-row">
              ...
              <span className="month-pill">{formatCurrency(MEAL_UNIT_PRICE)} / kişi</span>
            </div>
