...
function calculateMealAmount(headcount: number, invoiceRequested: boolean) {
  const netTotal = headcount * MEAL_UNIT_PRICE;
  const vatTotal = invoiceRequested ? netTotal * VAT_RATE : 0;
...
export function CateringDashboard() {
  ...
  const [reportInvoiceRequested, setReportInvoiceRequested] = useState(true);
  ...
  const reportAmounts = calculateMealAmount(reportTotalHeadcount, reportInvoiceRequested);
  const reportCompanyTotals = useMemo<ReportCompanyTotal[]>(() => {
    ...
    return [...totals.values()]
      .map((total) => {
        const amounts = calculateMealAmount(total.headcount, reportInvoiceRequested);
  ...
  }, [reportRequests, reportInvoiceRequested]);
      ...
      <section className="admin-main-panel">
        ...
        {adminView === "menu" ? (
          <section className="admin-section-panel menu-upload-panel">
            ...
            <form className="menu-upload-form" onSubmit={uploadMenuPdf}>
              <label>
                <span>Beklenen format</span>
                <input value="Tarih | Yemek 1 | Yemek 2 | Yemek 3 | Yemek 4" readOnly />
              </label>
              <label>
        ...
        {adminView === "reports" ? (
          <section className="admin-section-panel billing-report-panel">
            <div className="panel-title-row orders-panel-title">
              <div>
                <h2>Fatura ve ödeme raporu</h2>
                <p>{formatCurrency(MEAL_UNIT_PRICE)} kişi başı ücret üzerinden aylık firma toplamlarını hesapla.</p>
              </div>
              <div className="admin-toolbar monthly-toolbar">
                <label className="dashboard-date-filter">
                  <CalendarDays size={17} />
                ...
                <button className="invoice-toggle-button" type="button" onClick={() => setReportInvoiceRequested((current) => !current)}>
                  <CheckCircle2 size={17} />
                  {reportInvoiceRequested ? `Faturalı +%${VAT_RATE * 100} KDV` : "Faturasız ödeme"}
                </button>
            ...
            <div className="billing-summary-grid">
              ...
              <article>
                ...
                <small>{formatCurrency(MEAL_UNIT_PRICE)} x porsiyon</small>
              </article>
              <article>
                <span>KDV</span>
                <strong>{formatCurrency(reportAmounts.vatTotal)}</strong>
                <small>{reportInvoiceRequested ? `Fatura isteyene %${VAT_RATE * 100} KDV` : "Faturasız ödemede eklenmez"}</small>
              </article>
              <article className="highlight">
                <span>Tahsil edilecek</span>
                <strong>{formatCurrency(reportAmounts.grossTotal)}</strong>
                <small>{reportInvoiceRequested ? "Faturalı toplam" : "Faturasız toplam"}</small>
              </article>
            </div>
              ...
              {isReportLoading ? (
                ...
                reportCompanyTotals.map((companyTotal) => (
                  <div className="billing-company-row" key={companyTotal.companyId}>
                    ...
                    <span>{formatCurrency(companyTotal.vatTotal)}</span>
