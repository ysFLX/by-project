...
export function CateringDashboard() {
  ...
  const isServiceDateToday = serviceDate === todayKey();
  ...
  const collectionRows = useMemo(() => {
    const requestsByCompany = new Map(requests.map((request) => [request.companyId, request]));

    return companies
      .filter((company) => company.active)
      .map((company) => ({
        company,
        request: requestsByCompany.get(company.id) ?? null
      }))
      .sort((first, second) => {
        const statusOrder = { eaten: 0, submitted: 1, missing: 2, collected: 3 };
        const firstStatus = first.request?.status ?? "missing";
        const secondStatus = second.request?.status ?? "missing";
        ...
        return first.company.name.localeCompare(second.company.name, "tr-TR");
      });
  }, [companies, requests]);
  const collectionReadyCount = requests.filter((request) => request.status === "eaten").length;
  const collectionWaitingCount = requests.filter((request) => request.status === "submitted").length;
  const collectionDoneCount = requests.filter((request) => request.status === "collected").length;
  ...
  async function saveOperationSettings(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    ...
    try {
      const { settings } = await apiFetch<{ settings: OperationSettings }>("/operation-settings", {
        method: "PUT",
        body: operationSettings
      });

      setOperationSettings(settings);
      setMessage("Yemek yenildi ve toplama saatleri kaydedildi.");
      loadDashboard();
    } catch (settingsError) {
  ...
  function renderCollectionRow(company: ClientCompany, request: MealRequest | null) {
    const status = request?.status ?? "missing";
    const isReady = status === "eaten" && isServiceDateToday && nowIstanbulTime <= operationSettings.collectedDeadline;
    const StatusIcon = request ? statusMeta[request.status].icon : AlertTriangle;
    const statusLabel = request ? statusMeta[request.status].label : "Bildirim yok";
    const statusTone = request ? statusMeta[request.status].tone : "warning";

    return (
      <div className={`meal-request-row admin-order-row status-${status}`} key={company.id}>
        ...
        <span className={`order-status-pill tone-${statusTone}`}>
          <StatusIcon size={15} />
          {statusLabel}
        </span>

        <small className="order-time-cell">
          {request ? formatTime(request.eatenAt ?? request.updatedAt) : "-"}
        </small>

        {request ? (
          <button className="catering-secondary-button" type="button" onClick={() => markCollected(request.requestNo)} disabled={isSaving || !isReady}>
            <CheckCircle2 size={17} />
            {status === "collected" ? "Toplandi" : "Toplandi yap"}
          </button>
        ) : (
          <span className="empty-state compact">Bekleniyor</span>
        )}
      </div>
    );
  }
          ...
          <section className="admin-section-panel monthly-tracking-panel">
            <div className="panel-title-row orders-panel-title">
              ...
              <div className="admin-toolbar monthly-toolbar">
                ...
                <button className="admin-icon-button" type="button" onClick={loadDashboard} aria-label="Yemek toplama listesini yenile">
                  {isLoading ? <Loader2 size={18} /> : <RefreshCcw size={18} />}
                </button>
              </div>
            </div>
            ...
            <div className="collection-status-stack">
              <article>
                <span>Yemek yenildi bildirimi geldi</span>
                <strong>{collectionReadyCount + collectionDoneCount}</strong>
                <small>{collectionReadyCount} toplanabilir, {collectionDoneCount} toplandi</small>
              </article>
              <article>
                <span>Yemek yenildi bildirimi gelmedi</span>
                <strong>{collectionWaitingCount + collectionMissingCount}</strong>
                <small>{collectionWaitingCount} bekliyor, {collectionMissingCount} bildirim yok</small>
              </article>
            </div>
            ...
            <form className="collection-deadline-form" onSubmit={saveOperationSettings}>
              <label>
                ...
                <input
                  type="time"
                  value={operationSettings.eatenDeadline}
                  onChange={(event) => setOperationSettings((current) => ({ ...current, eatenDeadline: event.target.value }))}
                />
              </label>
              <label>
                <span>Toplandı son saati</span>
                <input
                  type="time"
                  value={operationSettings.collectedDeadline}
                  onChange={(event) => setOperationSettings((current) => ({ ...current, collectedDeadline: event.target.value }))}
                />
              </label>
              <button className="catering-primary-button" type="submit" disabled={isSettingsSaving}>
            ...
            <div className="collection-columns">
              <div className="collection-group">
                <div className="collection-group-title">
                  <div>
                    <h3>Yemek yenildi bildirimi gönderen şirketler</h3>
                    <p>{collectionNotifiedRows.length} firma bu grupta.</p>
                  </div>
                  <span>{collectionReadyCount} toplanabilir</span>
                </div>
                ...
                <div className="daily-company-orders">
                  ...
                  {collectionNotifiedRows.length === 0 ? (
                    <p className="empty-state compact">Henüz yemek yenildi bildirimi gönderen firma yok.</p>
                  ) : (
                    collectionNotifiedRows.map(({ company, request }) => renderCollectionRow(company, request))
                  )}
                </div>
              </div>

              <div className="collection-group pending">
                <div className="collection-group-title">
                  <div>
                    <h3>Yemek yenildi bildirimi göndermeyen şirketler</h3>
                    <p>Yemek yenildi butonuna basmayan veya bugün adet göndermeyen firmalar.</p>
                  </div>
                  <span>{collectionWaitingCount + collectionMissingCount} bekliyor</span>
                </div>
                ...
                <div className="daily-company-orders">
                  ...
                  {collectionPendingRows.length === 0 ? (
                    <p className="empty-state compact">Bildirim göndermeyen firma kalmadı.</p>
                  ) : (
                    collectionPendingRows.map(({ company, request }) => renderCollectionRow(company, request))
                  )}
                </div>
              </div>
            </div>
